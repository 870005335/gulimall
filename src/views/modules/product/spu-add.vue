        ...
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-table :data="spu.skus" style="width: 100%">
            <el-table-column label="属性组合">
              ...
              >
                <template v-slot="{row}">
                  <span style="margin-left: 10px">{{ row.attr[index].attrValue }}</span>
                </template>
              </el-table-column>
            </el-table-column>
            <el-table-column label="商品名称" prop="skuName">
              <template v-slot="{row}">
                <el-input v-model="row.skuName"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="标题" prop="skuTitle">
              <template v-slot="{row}">
                <el-input v-model="row.skuTitle"></el-input>
              </template>
            </el-table-column>
            <el-table-column label="副标题" prop="skuSubtitle">
              <template v-slot="{row}">
                <el-input v-model="row.skuSubtitle"></el-input>
              </template>
            </el-table-column>
              ...
              <template v-slot="{row}">
                    ...
                    >
                      ...
                      <div style="padding: 14px;">
                        <el-row>
                          <el-col :span="12">
                            <el-checkbox
                                v-model="row.images[index].imgUrl"
                                :true-label="img"
                                false-label
                            ></el-checkbox>
                          </el-col>
                          <el-col :span="12">
                            <el-tag v-if="row.images[index].defaultImg === 1">
                              <input
                                  type="radio"
                                  checked
                                  :name="row.skuName"
                                  @change="checkDefaultImg(row,index,img)"
                              />设为默认
                            </el-tag>
                            <el-tag v-else>
                              <input
                ...
                <el-form :model="row">
                  <el-row>
                    ...
                    <el-col :span="24">
                      <el-form-item label="设置会员价" v-if="row.memberPrice.length>0">
                        <br />
                        <!--   @change="handlePriceChange(scope,mpidx,$event)" -->
                        <el-form-item v-for="(mp,mpidx) in row.memberPrice" :key="mp.id">
                          {{mp.name}}
                          <el-input-number
                              style="width:160px"
                              v-model="row.memberPrice[mpidx].price"
                              :precision="2"
                              :min="0"
                              controls-position="right"
                          ></el-input-number>
  ...
  methods: {
    ...
    showBaseAttrs() {
      if (!this.dataResp.steped[0]) {
        this.$http.get(
            `/product/attr/group/withAttr/${this.spu.categoryId}`
        ).then(({ data: res }) => {
          //先对表单的baseAttrs进行初始化
